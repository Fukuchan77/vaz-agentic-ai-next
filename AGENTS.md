...
- **`docs/cross-repo-adoption-review.md`** is the canonical cross-repo review (X-1–X-16) that `fastapi-pydantic-ai-agent`, `pydantic-ai-sandbox`, and this repo's own `docs/cross-repo-adoption-backlog.md` cite as the source of truth. It was missing for months (the reviewing sessions could never find it); it's placed here now, verbatim (493 lines, unmodified — see the append-only convention for point-in-time review documents), with a dated `## §6` addendum recording the identity collapse and re-measured rows (notably: Actions SHA pinning was 0/22 when the review measured it, is 28/28 now — X-1 has landed).
- **`specs/memory/constitution.md`** (11 principles, v1.2.0) is this repo's constitution, inherited from the prior `vaz-agentic-ai-next` (now archived at `vaz-agentic-ai-next-archive` — the name was freed by an `-archive` rename, not a delete, precisely because that repo turned out to hold ~5,400 lines of non-`main`-branch content the consolidation plan had assumed didn't exist). The canonical review cites this constitution normatively ("principle 8", "the multi-agent adoption gate in §5.1") — those citations now resolve.
- **`specs/inherited/001-agentic-ai-core-p0/`** is a reference-only spec inherited from the same source: 53 approved requirements (chat / HITL / RAG / observability / CI) that remain valid product requirements. Its assembly mechanism does not: it was designed in the opposite direction (a new repo importing `vaz-ai-next` as a dependency, per its own `spec.json.repository.note`), and is superseded by [`docs/adr/0003-consolidation-direction.md`](docs/adr/0003-consolidation-direction.md), which also fixes where a second Python lane goes (`services/api`, never `apps/agent-api`) and why no root `pyproject.toml`/`turbo.json` gets created. Don't renumber it into the `001`–`006` spec sequence — it collides with this repo's own `001-vaz-ai-update`.
- Full narrative, the three corrected assumptions, and the salvage/rename operational record live in `specs/006-repo-consolidation/` (`spec.md`, `plan.md`, `tasks.md`, `pdca/do.md`).
- **[`docs/guide/`](docs/guide/README.md)** is the learning-path guide built on top of this — one page per technique in `docs/agentic-engineering-review.md`'s eight-technique skeleton (PE/CE/LE/HE/AE/AO/MCP/EV), each linking (a) this repo's implementation, (b) sibling-repo teaching material (including the book manuscript in `pydantic-ai-agentic-patterns`), and (c) the canonical review's relevant X-n items. No prose is duplicated there — it's links plus a 1–3 sentence frame, per constitution principle 5 ("join the existing single path").
...
- **`job_step` table** (`packages/db/src/schema.ts`, migration `packages/db/drizzle/0002_add_job_step.sql`) — tracks per-step pending/consumed state and cumulative token usage across suspend/resume boundaries. Composite PK `(job_id, step_id)` enforces consume-once uniqueness at the DB layer. `approval_state` pg enum values: `pending` / `consumed`. `total_tokens` is an absolute observed value (not an increment) — cumulative budget is obtained via `SUM`. `jobEventTypeEnum` is unchanged (SSE backward-compatibility invariant).
- **`POST /api/jobs/:id/approve` is no longer fire-and-forget** (spec `007-cross-repo-adoption-closeout`) — it now validates via `lib/approvals.ts` before sending the resume signal. New response codes: `400` (schema violation / strict-object excess fields / bad JSON), `404` (approval target not found / not claimable — 3 cases collapsed to one status to prevent enumeration), `409` (duplicate `toolCallId` in decision set / set contains non-pending target), `429` (per-job token budget exceeded). The existing `400/401/404/403` authorization ladder (`authorizeJobAccess`) is unchanged. Approval requests accept a single decision `{ toolCallId, decision, args? }` or a decision set `{ decisions: [...] }` — both forms use `z.strictObject`; sending `history`, `messages`, `usage`, or `model` fields results in `400`.
...
- **Three majors are deliberately held back** (re-measured 2026-09-19 during the dependency refresh; each is a decision, not a stale range):
...
- **Vitest projects** — root `vitest.config.ts` aggregates four projects: `web` (jsdom, `apps/web/tests/**`), `worker` (node, `apps/worker/tests/**`), `packages` (node, `packages/*/tests/**` + `packages/*/src/unit/**`), and `repo` (node, `tests/repo/**`). Coverage (Vitest 4 semantics: only files loaded during the run are reported unless `include` adds them) measures the whole workspace — `apps/web/src/**`, `apps/worker/src/**`, `packages/*/src/**` — excluding unit-untestable entry points: App Router entries (`apps/web/src/app/**`, E2E territory), stylesheets, pure barrels, and process/CLI mains (`apps/worker/src/start.ts`, `packages/evals/src/nightly.ts`). Thresholds: lines/functions ≥ 80%.
- **Repo-governance guards** live in `tests/repo/` (the `repo` Vitest project, not tied to one package): `ci-workflows.spec.ts` (every `uses:` SHA-pinned + every workflow declares `permissions:`, X-1), `dependabot.spec.ts` (X-15), `hermetic-network.spec.ts` (the `fetch` guard in `tests/setup/hermetic-network.ts` is live, X-2), `cross-repo-reference-resolution.spec.ts` (the canonical review resolves and no reference names a pre-rename repo, R9.1), `model-id-gate-precision.spec.ts` (the shell gate and `services/api`'s own test don't drift apart), `doc-links.spec.ts` (every relative Markdown link resolves), `owasp-mapping-citations.spec.ts` (citation paths + symbols + CI step names + 3-value vocabulary + taxonomy version dates in both OWASP mapping docs, X-20), and `egress-policy-bypass.spec.ts` (email-literal and allowlist-bypass scan across `apps/*/src/**` + `packages/*/src/**`; audit-sink single firing-point assertion, D6). Each asserts "scanned > 0" first — a guard that silently scans nothing is the failure mode they exist to prevent. Two writing rules follow from the last two: a document in this repo is linked (so `doc-links.spec.ts` keeps the pointer honest — a "canonical" source that only ever appeared as a code span is how the cross-repo review stayed missing for months), while a path in another repository stays a code span (`doc-links.spec.ts` cannot reach it and would flag the link). `doc-links.spec.ts` skips vendored subtrees — `services/api` is a verbatim `git subtree` import whose Markdown is upstream's to fix — and asserts each skipped root still exists so the exclusion can't go stale.

...
| コンテキスト予算の段階導入設計 | `docs/guide/context-engineering.md`（178 行） | Stage 0（全履歴＋停止述語）→ Stage 1（`prepareStep` 窓化シーム、**未指定時 byte 等価**）→ Stage 2（自動 compaction）。`totalTokens` が reasoning を含みうるため `inputTokens + outputTokens` を自前合算する理由まで記録 |
| AgentOps ランブック | `docs/guide/agentops.md`（175 行） | 可観測性・評価・最適化の 3 本柱を実コードへ写像。**未実装の項目を要件 ID 付きで明示する**書き方（実装済みのように描く事故を避ける） |

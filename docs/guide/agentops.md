...
自律エージェントのライフサイクル管理を、**可観測性・評価・最適化**の 3 本柱で運用するための
正本。定義と AO-1〜AO-5 は
[`docs/agentic-engineering-review.md`](../agentic-engineering-review.md) §1.6 を参照する。

このページは学習ガイドとリポジトリ固有の運用マッピングを兼ねる。旧
`docs/agentops.md` の詳細と旧 `docs/guide/agentops.md` の案内を統合し、同じ主題の正本を 1 つにした。
...
描く事故を避ける。[`context-engineering.md`](./context-engineering.md) と同じ方針）。
...
併記される（下記「Req 1.4 metrics」節、[`context-engineering.md`](./context-engineering.md) の該当節と同一の記述を参照）。
...
されない。詳細な方針とチャット本線での挙動は [`context-engineering.md`](./context-engineering.md) の
...
- **閾値付きの cost-latency ダッシュボードは未実装。** `pr-gate.ts` / `nightly.ts` が算出する
  ...
  将来実装する場合は、[`context-engineering.md`](./context-engineering.md) の Stage 0（OTel run-metrics の実測に基づく
...
- [`context-engineering.md`](./context-engineering.md) — チャット本線のコンテキスト予算・停止理由監査の詳細（本文書の
  「可観測性」節が参照する正本）。
- [`docs/adr/0001-mcp-position.md`](../adr/0001-mcp-position.md) — MCP 採用判断（Req 6.2/6.3、Task 12.2）。

# コンテキストエンジニアリング (CE) — コンテキスト予算方針

推論時にモデルが見るトークン全体の構成と予算を管理するための正本。定義と CE-1〜CE-5 は
[`docs/agentic-engineering-review.md`](../agentic-engineering-review.md) §1.2 を参照する。

このページは学習ガイドと実装方針を兼ねる。旧 `docs/context-budget.md` の詳細と旧
`docs/guide/context-engineering.md` の案内を統合し、段階的 compaction と関連教材の参照先を
1 か所に集約した。

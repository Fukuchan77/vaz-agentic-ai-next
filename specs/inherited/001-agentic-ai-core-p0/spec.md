...
**一次情報源**: `specs/001-agentic-ai-core-p0/spec-agenticai-core.md`（v1.7）
...
## 横断検証の反映（2026-09-08 / 一次情報源 v1.7）

`docs/cross-repo-adoption-review.md`（5 リポジトリ相互取り込み検証、2026-09-06）が「本リポジトリは実装前に仕様へ反映する」と定めた 4 項目を、一次情報源 v1.7 に反映した（同 §0.3.1 の対応表を参照）。

| 項目 | 反映先（一次情報源） | P0 への影響 |
| :--- | :--- | :--- |
| X-5 停止理由語彙の写像（Python 5 値 / TS 4 値） | §9.2.2 / §12 R15 / §11 P1 | **なし**（P1 の受け入れ条件） |
| X-10 SSE の第 3 の罠（U+2028 / U+2029 と `str.splitlines()`） | §4.4.1 / §11 P3 | **なし**（P3 の受け入れ条件） |
| X-13 OWASP 対応表（LLM Top 10 ＋ Agentic AI） | §6.5（新設） | **なし**。ただし LLM03 行が引用するテストは REQ-9.3 / REQ-9.2 であり、**P0 が納品する** |
| X-14 E2E CI レーン | §10.2（v1.6 で既述）／§11 P3 の前提として明示 | **なし**（P0 スコープに `test:e2e` は含まない — REQ-9.1 の CI 検査表のとおり） |

**本要件セクション（P0 の 53 要件）は増減していない。** 反映先はいずれも P1 以降の受け入れ条件と設計記述であり、`tasks.md` の T-0〜T-9 および `traceability.md` のマトリクスも変更を要しない。したがって `spec.json` の `approvals` は P0 デリバリ単位に対して有効なまま維持する。

なお X-2（ソケット遮断を `connect` だけでなく `connect_ex` / `getaddrinfo` にも広げる）は一次情報源 §8.2 の設計記述として追記した。**REQ-7.4 の条文は変更していない**（要件の強化は P0 の承認済み受け入れ条件を動かすため、別サイクルで判断する）。

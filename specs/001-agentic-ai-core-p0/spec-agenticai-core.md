...
- **加えてソケットレベルでネットワークを遮断する**。`fastapi-pydantic-ai-agent` は `tests/support/hermetic.py::block_network()` を autouse fixture で `tests/unit/` 全体に適用し、`AF_INET` / `AF_INET6` の `socket.connect` を `NetworkBlockedError` にする。モック漏れが即座に落ちる
...
- https://github.com/Fukuchan77/pydantic-ai-sandbox — `patterns/hitl/` / `patterns/sse/` / `patterns/contracts/` / `patterns/deep-research/` / `patterns/frameworks/{pydantic-ai,llamaindex,beeai}/` / `specs/012-agentic-ai-design/` / `specs/013-agentic-ai-security/` / `specs/document-review/agentic-ai-design-v2-review.md` / **`patterns/SECURITY-NOTES.md` の「上限ピンによる脆弱版回避の運用」節**と `pyproject.toml` の **ADR-2**（§2.6 の一次出典）
